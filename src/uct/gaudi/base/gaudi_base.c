...
static void uct_gaudi_base_get_sys_dev_from_fd(int fd, int index,
                                               ucs_sys_device_t *sys_dev_p)
{
    ucs_status_t status;
    struct hlthunk_hw_ip_info hw_ip;
    const unsigned sys_device_priority = 100;
    char device_name[16];
    char pci_bus_id[13];
    int rc;

    if (fd < 0) {
        ucs_error("invalid fd %d", fd);
        goto err;
    }

    rc = hlthunk_get_pci_bus_id_from_fd(fd, pci_bus_id, sizeof(pci_bus_id));
    if (rc) {
        ucs_error("failed to get pci bus id from fd %d (rc=%d)", fd, rc);
        goto err;
    }

    status = ucs_topo_find_device_by_bdf_name(pci_bus_id, sys_dev_p);
    if (status != UCS_OK) {
        ucs_error("pci %s not found in topology", pci_bus_id);
        goto err;
    }
...
ucs_status_t uct_gaudi_base_discover_devices(void)
{
    ...
    for (i = 0; i < HLTHUNK_MAX_MINOR; i++) {
        ...
        uct_gaudi_base_get_sys_dev_from_fd(fd, discovered_devices, &sys_dev);
        close(fd);
