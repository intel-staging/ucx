...
uct_ze_ipc_iface_event_arm(uct_iface_h tl_iface, unsigned events)
{
    ...
    for (i = 0; i < iface->num_cmd_lists; i++) {
        ...
        ucs_queue_for_each_safe(event_desc, &q_desc->event_queue,
                                uct_ze_ipc_event_desc_t, queue) {
            ret = zeEventQueryStatus(event_desc->event);
            if (ret != ZE_RESULT_NOT_READY) {
                return UCS_ERR_BUSY;
            }
                           ...
                           const uct_iface_config_t *tl_config)
{
    ...
    md     = ucs_derived_of(uct_md, uct_ze_ipc_md_t);
    ...
    self->ze_context  = md->ze_context;
    self->ze_device   = md->ze_device;
    self->config      = *config;
...
static UCS_CLASS_CLEANUP_FUNC(uct_ze_ipc_iface_t)
{
    ...
    for (i = 0; i < self->num_cmd_lists; i++) {
        ...
        ucs_queue_for_each_safe(event_desc, &self->queue_desc[i].event_queue,
                                uct_ze_ipc_event_desc_t, queue) {
            /* wait for event */
    ...
    if (self->pidfd_cache != NULL) {
        for (iter = kh_begin(self->pidfd_cache);
            ...
            if (kh_exist(self->pidfd_cache, iter)) {
                pidfd = kh_value(self->pidfd_cache, iter);
                pid   = kh_key(self->pidfd_cache, iter);
                close(pidfd);
